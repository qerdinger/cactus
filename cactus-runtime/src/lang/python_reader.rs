...
impl PythonReader {
    ...
    fn convert_idx_to_argument(s : &str, s_len: usize, s_at: usize) -> Option<Argument> {
        ...
        for (i, c) in s[l_parenthesis.unwrap() + SPACE_LENGTH .. r_parenthesis.unwrap()].chars().enumerate() {
            print!("{}", c);
        }
        None
    }
    ...
    fn extract(&self, fragment: &Fragment) -> Vec<Function> {
        ...
        let functions: Vec<_> = fnc_indexes.iter().map(|x| {
            (
                ...
                Self::convert_idx_to_argument(fragment.raw_data(), content_size, *x)
            )
        }).collect();
